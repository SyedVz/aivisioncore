...
CamList = ['data/out_truck.mp4', 'data/nighttime_2.mp4', 'data/fog_SCAM011_20240507-033640.mp4','data/traffic_KCAM030_20240507-103904.mp4']
# CamList = ['PCAM4061']
...
aspect_wide, aspect_tall, scl = 580, 650, 1.00
vid_h = aspect_tall
compos_h, compos_w, mgn, dx = int(vid_h * scl), int(1080 * scl), 8, int(np.ceil(math.sqrt(len(CamList))))
preview_h, preview_w = compos_h // dx - mgn, compos_w // dx - mgn
head_top, head_left = int(compos_h*0.2), preview_w+mgn+int(compos_h*0.2)
compos_w += head_left
compos_h += head_top
...
#alert_queries = ['traffic congestion', 'flood','fog','accident','vehicle crash','explosion']
...
class QueryWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Live Traffic Queries")
        ...
        self.threshold_spin.setValue(alert_thresh)
        ...
        list_layout.addWidget(self.query_list)
        list_layout.addWidget(self.remove_button)
        layout.addLayout(list_layout)

        # Add e
        for query in alert_queries:
            if query not in alert_exclude:
                self.query_list.addItem(query)
    ...
    def add_query(self):
        ...
        if query and query not in alert_queries:
    ...
    def remove_query(self):
        ...
        for item in selected_items:
            query = item.text()
            if query in alert_queries and query not in alert_exclude:
                alert_queries.remove(query)
...
if __name__ == "__main__":
    ...
    interface_thread = threading.Thread(target=run_query_interface)
